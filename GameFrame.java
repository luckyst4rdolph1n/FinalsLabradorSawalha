...
public class GameFrame implements Runnable{
    ...
    */
    ...
    public void createPlayers(){
        if(playerID == 1){ //first player to enter
            player1 = new Player(100, 530, "/resources/Asset9.png"); //dog
            player2 = new Player(300, 530, "/resources/frog2.png");
        }else{ //second player to enter
            player2 = new Player(100, 530, "/resources/Asset9.png"); 
            player1 = new Player(300, 530, "/resources/frog2.png"); //frog
        }
    }
    ...
    public void createGameScene(){
        ...
        baseGround = new Platforms("/resources/groundBase.png", 0, 570, 800, 31); //ground
        //platrforms
        ...
        flag = new Flag(); //flag for finish line
    }

    /**
     * adds the platforms to the scene
     */
    ...
    public void connectToServer(){
        try{
            ...
            System.out.print("IP Address: "); //prompts the user to enter the IP Address
            String host = inp.nextLine();
            System.out.print("Enter port: "); //prompts the user to enter the port number
            ...
            if(playerID ==1){
                System.out.println("Waiting for Player #2 to connect..."); //while waiting for player 2
            }
    ...
    private class ReadFromServer implements Runnable {
        ...
        public void waitForStartMsg(){ //waits for the starting message before the game starts for both sides
            try{
    ...
    public ArrayList<Platforms> Colliding(Player player, ArrayList<Platforms> ps){
        ...
        for(Platforms a: ps){
            if(checkCollision(a, player)){
                collidingPlatforms.add(a); //if there is collision, add to the arraylist
            }
        }
    ...
    public void platformCollisions(ArrayList<Platforms> pfs , Player player){
        ...
        if(collidingPfs.size() > 0){//if there is colission
            Platforms collided = collidingPfs.get(0);
            if(player.vely>0){
                player.setBottom(collided.getTop()); //gets the top part of the platform and sets the player on it
            }else if(player.vely<0){
                player.setTop(collided.getBottom()); //gets the bottom part of the platform and limits the player's jump
            }
            player.vely = 0;
        }
    ...
    public void breezyCollisions(ArrayList<Breezy> br , Player player){
        for(int i=0; i<br.size(); i++){
            if(checkCollision(br.get(i), player)){
                br.remove(i); //removes Breezy when there is collision
                new Thread(new Runnable()
                {
                    ...
                    public void run()
                    {
                        player.speed += 10;
                        try {
                            Thread.sleep(3000); //after three seconds, the power up wears off
                        } catch (InterruptedException e) {
                            e.printStackTrace();
                        }
                        player.speed -= 10;
